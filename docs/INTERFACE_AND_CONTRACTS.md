...
| `live.semi_auto_checklist` | `build_semi_auto_checklist(...)`、`save_semi_auto_checklist(settings, strategy, trade_date, checklist, decision)` | 冻结清单路径、纸面日报路径、运行监控表、风险总控表、人工确认单、成交回填、表现归因、偏差分析；成交回填和表现归因可按参数设为必需 | 半自动执行清单、最终决策和 Markdown 报告，默认写 `output/semi_auto_checklist/<strategy>/<date>_semi_auto_checklist.csv`、`<date>_semi_auto_decision.csv`、`<date>_semi_auto_checklist.md` |
| `live.live_sop` | `build_live_daily_sop(settings, strategy, trade_date, ...)`、`save_live_daily_sop(settings, strategy, trade_date, sop)` | `Settings`、策略名、交易日期、可选冻结清单路径、可选券商持仓路径、是否纳入只读对账步骤 | 小资金实盘每日 SOP DataFrame 与 Markdown 报告，默认写 `output/live_sop/<strategy>/<date>_daily_sop.csv`、`<date>_daily_sop.md`；只生成操作顺序，不下单、不修改账户 |
...
| `scripts/build_semi_auto_checklist.py` | CLI | 默认读取冻结清单、运行监控、风险总控、人工确认单、纸面日报、成交回填、表现归因和偏差分析；可用参数显式指定 | 调用 `live.semi_auto_checklist`，生成 `READY_FOR_MANUAL_ORDER` / `MANUAL_REVIEW` / `DO_NOT_TRADE` 总决策 |
| `scripts/build_live_sop.py` | CLI | `--strategy`、`--trade-date`、可选 `--freeze-manifest`、`--broker-positions`、`--include-broker-reconcile` | 调用 `live.live_sop`，生成当天小资金实盘 SOP CSV / Markdown，用于人工执行和复盘 |
